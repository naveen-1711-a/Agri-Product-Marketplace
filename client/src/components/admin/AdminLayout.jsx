...
import { FiHome, FiUsers, FiPackage, FiShoppingBag, FiBarChart2, FiMenu, FiX, FiLogOut, FiTool } from 'react-icons/fi';
import { GiWheat } from 'react-icons/gi';
import { useAuth } from '../../context/AuthContext';

const NAV = [
  { to: '/admin/dashboard', icon: FiHome, label: 'Dashboard' },
  { to: '/admin/sellers', icon: FiUsers, label: 'Sellers' },
  { to: '/admin/products', icon: FiPackage, label: 'Products' },
  { to: '/admin/equipment', icon: FiTool, label: '🚜 Equipment' },
  { to: '/admin/orders', icon: FiShoppingBag, label: 'Orders' },
  { to: '/admin/customers', icon: FiBarChart2, label: 'Customers' },
];
    ...
    <div className="h-full flex flex-col bg-primary-dark text-white">
      <div className="p-5 border-b border-primary-700 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div><p className="font-bold text-sm">EPM Admin</p><p className="text-xs text-primary-light">Control Panel</p></div>
        </div>
        {onClose && (
      ...
      <div className="p-4 border-t border-primary-700">
        <div className="flex items-center gap-3 mb-3 px-1">
          ...
          <div className="min-w-0"><p className="text-sm font-medium truncate">{user?.name}</p><p className="text-xs text-primary-light">Admin</p></div>
        </div>
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white shadow-sm px-4 sm:px-6 py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 lg:hidden">
              ...
              <span className="font-bold text-primary-dark text-base">EPM Admin</span>
            </div>
            <h1 className="font-semibold text-gray-700 text-lg hidden lg:block">Admin Dashboard</h1>
          </div>
          <span className="bg-primary-100 text-primary-dark text-xs font-semibold px-3 py-1 rounded-full">Admin</span>
        </header>
        ...
        {/* Mobile Bottom Nav */}
        <nav className="lg:hidden bg-white border-t border-gray-200 flex items-center justify-around py-2 px-2 flex-shrink-0 shadow-lg">
          {NAV.slice(0, 5).map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} end={to === '/admin/dashboard'}
              className={({ isActive }) => `flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg text-xs transition-colors ${isActive ? 'text-primary-700 font-semibold' : 'text-gray-500'}`}>
              <Icon size={20} />
              <span className="leading-none">{label.replace('🚜 ', '')}</span>
            </NavLink>
          ))}
        </nav>
      </div>
